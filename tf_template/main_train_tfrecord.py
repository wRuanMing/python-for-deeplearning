# -*- coding: utf-8 -*-
"""
Created on Mon May  7 13:58:45 2018
...
def parse_args():
    ...
    parser.add_argument('--datasetname', default='data', type=str)
...
#    if command_args.istrain==True:
#        loaddatadir = os.path.join(command_args.datadir,'train')
#    elif command_args.istrain==False:
#        loaddatadir = os.path.join(command_args.datadir,'test')
    loaddatadir = os.path.join(command_args.datadir, command_args.trainortest)
    print(loaddatadir)
    imgs_dict = get_imgsnameandlabels_from_dataset_folder(loaddatadir)        
    ## mnist数据
    ...
    df = pd.DataFrame({'name':['image','label'],
                      'type':['float32','int64'],
                      'shape':[(command_args.imgchannel*command_args.imgsize**2,),()],
                      'isbyte':[False,False],
    ...
    dataset_record_name = os.path.join(command_args.datadir,command_args.datasetname+'_tfrecord')
    if not os.path.exists(dataset_record_name):
        os.mkdir(dataset_record_name)
    if not os.path.exists(os.path.join(dataset_record_name,loaddatadir.split('\\')[-1])):
        os.mkdir(os.path.join(dataset_record_name,loaddatadir.split('\\')[-1]))
    
    # 用该方法写训练集的tfrecord文件
    #dataset = mnist.train
    path = os.path.join(dataset_record_name,loaddatadir.split('\\')[-1],loaddatadir.split('\\')[-1])
    ...
    for i in np.arange(len(imgs_dict["img_name"])):
        ...
        tfr.feature_writer(df.iloc[0], Image.open(imgs_dict["img_name"][i]).resize((command_args.imgsize,command_args.imgsize)).tobytes(), features)
        ...
        print('load image : %s of %s' %(i+1, len(imgs_dict["img_name"])))
    ...
    data_info_path = os.path.join(dataset_record_name,'data_info.csv')
#    data_info_path = os.path.dir(dataset_record_name,'data_info.csv')
